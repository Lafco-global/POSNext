...
DOCTYPE_POS_SETTINGS = "POS Next Settings"
...
def _should_block(pos_profile):
	...
	if pos_profile:
		...
		pos_settings_allow_negative = cint(
			frappe.db.get_value("POS Next Settings", {"pos_profile": pos_profile}, "allow_negative_stock")
			or 0
		)
...
def validate_return_items(original_invoice_name, return_items, doctype="Sales Invoice"):
	...
	if invoice_info.pos_profile:
		return_validity_days = cint(
			frappe.db.get_value(
				"POS Next Settings", {"pos_profile": invoice_info.pos_profile}, "return_validity_days"
			)
			or 0
		)
...
def get_returnable_invoices(limit=50, pos_profile=None):
	...
	if pos_profile:
		return_validity_days = cint(
			frappe.db.get_value("POS Next Settings", {"pos_profile": pos_profile}, "return_validity_days")
			or 0
		)
